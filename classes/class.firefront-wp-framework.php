...
/*
 * Firefront WP Framework Class
 */

class Firefront_Wp_Framework {
	
	private static $_instance = null;

	
	public static function firefront_mobile_logo() {
		...
		$logo_values = Firefront_Wp_Elements::firefront_get_meta_and_option_values( $logo_keys );
		extract( $logo_values );
	...
	public static function firefront_site_logo( $sticky = false ) {
		$site_logo = '';
		...
		if( !empty( $site_logo ) && is_array( $site_logo ) && isset( $site_logo['image'] ) ){
			...
			if ( wp_attachment_is_image( $site_logo_id ) ) {
				...
				if( !$sticky ) {
					Firefront_Wp_Framework::firefront_basic_logo();
				}
			}
		}
	}
	
	public static function firefront_sticky_logo() {
		...
		if( !empty( $sticky_logo ) && is_array( $sticky_logo ) && isset( $sticky_logo['image'] ) ){
			...
			if ( wp_attachment_is_image( $sticky_logo_id ) ) {
				...
				Firefront_Wp_Framework::firefront_site_logo(true);
			}
		}
	}

	public static function firefront_basic_logo( $args = array(), $head_tag = 'h1' ) {
		$site_title = get_bloginfo( 'name' );
		...
		$contents   = '';
		$classname  = '';
		//$page_title_opts = Firefront_Wp_Elements::firefront_options( 'page-title-enable' );
		$current = '';
		...
		$args = apply_filters( 'firefront_site_logo_args', $args, $defaults );
		if ( has_custom_logo() ) {
		...
		echo apply_filters( 'firefront_site_logo', $html, $args, $classname, $contents );
	}

	public static function firefront_site_description() { //Firefront_Wp_Framework::firefront_site_description()
		$logo_keys = array(
			'chk' => 'logo-chk',
			'fields' => array(
		...
		if( isset( $logo_desc ) && $logo_desc ){
			...
			echo apply_filters( 'firefront_site_description', $html, $description, $wrapper );
		}
	}

	public static function firefront_get_the_terms_as_out( $post_id, $term_slug ){ //Firefront_Wp_Framework::firefront_get_the_terms_as_out
	...
	public static function firefront_get_post_date_as_link(){ //Firefront_Wp_Framework::firefront_get_post_date_as_link
	...
	public static function firefront_get_post_author(){ //Firefront_Wp_Framework::firefront_get_post_author
	...
	public static function firefront_get_post_comment_count(){ //Firefront_Wp_Framework::firefront_get_post_comment_count
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="bi bi-chat-left-dots"></span> ';
			comments_popup_link( esc_html__( 'No Comments', 'firefront' ), esc_html__( '1 Comment', 'firefront' ), esc_html__( '%s Comments', 'firefront' ) );
		}
	}
	
	public static function firefront_email_link( $email, $target = '_self' ){ 
		if( $email )
	...
	public static function firefront_address( $address ){ 
		if( $address ){
			echo '<address><span class="bi bi-geo-alt theme-color me-1"></span>'. nl2br( $address ) .'</address>';
		}
	}
	
	public static function firefront_fullbar_search_form(){ ?>
	...
	public static function firefront_overlay_search_form(){
	?>
	...
	public static function firefront_textbox_search_form(){
	?>		
		<div class="textbox-search-wrap">
			<form method="get" class="search-form" action="<?php echo esc_url( home_url( '/' ) ); ?>">
				<div class="input-group">
					<input name="s" type="text" class="form-control" value="<?php echo get_search_query(); ?>" placeholder="<?php echo apply_filters( 'firefront_search_placeholder', esc_attr__( 'Search and hit enter..', 'firefront' ) ); ?>">
				</div>
			</form>
		</div>		
	<?php
	...
	public static function firefront_search_modal( $serach_opt, $part = 'navbar' ){
		switch( $serach_opt ){
		
			case '1': ?>
				<a class="full-search-toggle" href="#"><i class="bi bi-search"></i></a>
			<?php
				add_action( 'firefront_footer_after', array( 'Firefront_Wp_Framework', 'firefront_overlay_search_form' ), 10 );
			break;
			
			case '2': 
				self::firefront_textbox_search_form();
			?>				
			...
			<?php
				add_action( 'firefront_'. esc_attr( $part ) .'_after', array( 'Firefront_Wp_Framework' , "firefront_fullbar_search_form" ) , 10 );
			break;
...
Firefront_Wp_Framework::get_instance();